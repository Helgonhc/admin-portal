  ...
  async function handleRespond() {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          status: newStatus,
          confirmed_date: responseForm.confirmed_date,
      ...
      <div className="space-y-4">
        {filteredRequests.length === 0 ? (
          ...
          filteredRequests.map((request) => (
            <div key={request.id} className={`card border-l-4 ${
              request.display_status === 'atrasado' ? 'border-l-red-500 bg-red-50' :
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <p className="font-semibold text-gray-800">
                        {new Date(request.suggested_date).toLocaleDateString('pt-BR')}
                      </p>
                      <p className="text-xs text-gray-500">{getTimePeriodLabel(request.suggested_time_period)}</p>
                    </div>
                    {request.confirmed_date && (
                      <div className={`rounded-lg p-3 ${
                        request.status === 'confirmado' ? 'bg-emerald-50' : 'bg-blue-50'
                      }`}>
                        <p className="text-xs text-gray-500 mb-1">
                          {request.status === 'confirmado' ? 'Data Confirmada' : 'Nova Data Sugerida'}
                        </p>
                        <p className={`font-semibold ${
                          request.status === 'confirmado' ? 'text-emerald-700' : 'text-blue-700'
                        }`}>
                          {new Date(request.confirmed_date).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      {request.client_phone && (
                        <button
                          onClick={() => {
                            ...
                            const msg = `Olá ${request.requester_name || request.client_name}! Recebemos sua solicitação de manutenção para ${new Date(request.suggested_date).toLocaleDateString('pt-BR')}. Vamos analisar e confirmar em breve.`;
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                ...
                <p className="text-lg font-bold text-amber-800">
                  {new Date(selectedRequest.suggested_date).toLocaleDateString('pt-BR')}
                </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 transition-all text-center ${
                      responseForm.action === 'confirmar'
                        ? 'border-emerald-500 bg-emerald-50'
                        : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
                    <Check className={`w-6 h-6 mx-auto mb-2 ${
                      responseForm.action === 'confirmar' ? 'text-emerald-600' : 'text-gray-400'
                    }`} />
                  ...
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 transition-all text-center ${
                      responseForm.action === 'reagendar'
                        ? 'border-blue-500 bg-blue-50'
                        : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
                    <Calendar className={`w-6 h-6 mx-auto mb-2 ${
                      responseForm.action === 'reagendar' ? 'text-blue-600' : 'text-gray-400'
                    }`} />
